...
kLUT=6
best_lut_blif = "best_blifs/" + design[:-5] + "_bs_ep_temp_LUT%s.blif" % str(kLUT)
temp_blif = "temp_blifs/" + design[:-5] + "_bs_ep_temp.blif"
lib_path = "gen_newlibs/"

start=time.time()
abc_cmd = "read %s;read %s; st; resyn;dch ;st;resyn;dch;if -K %d; ps " % (genlib_origin, design, kLUT)
...
# Mapper call
def technology_mapper(genlib_origin, partial_cell_library, write_output=""):
    ...
    out_gen.close() 
    if write_output=="":
        abc_cmd = "read %s;read %s; st; dch;map -a; st;dch;st; if -K %d; ps " % (output_genlib_file, design, kLUT)
    else:
        abc_cmd = "read %s;read %s; st; dch;map -a; st;dch;st; if -K %d; ps ; write_blif %s" % (output_genlib_file, design, kLUT, write_output)
    res = subprocess.check_output(('abc', '-c', abc_cmd))
    #print(res)
...
for i in range(num_iterations):
    ...
    print("[inter: %d]get the best delay=%.1f area=%.1f; Mapped blif saved at %s" % ( i, delay, area, best_lut_blif))
    print(best_cells)
    delay, area = technology_mapper(genlib_origin, best_cells,)
    mab.update_batch(batch_actions, batch_rewards)

delay, area = technology_mapper(genlib_origin, best_cells,best_lut_blif)
print("[final: %d]get the best delay=%.1f area=%.1f; Mapped blif saved at %s" % ( i, delay, area, best_lut_blif))
print(best_cells)
end=time.time()
runtime=end-start
