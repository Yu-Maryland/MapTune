...
start=time.time()
abc_cmd = "read %s;read %s; st; resyn;dch ;st;resyn;dch;if -K 6; ps " % (genlib_origin, design )
...
def technology_mapper(genlib_origin, partial_cell_library):
    ...
    abc_cmd = "read %s;read %s; st; dch;map -a; st;dch;st; if -K 6; ps " % (output_genlib_file, design) 
    res = subprocess.check_output(('abc', '-c', abc_cmd))
    #print(res)
