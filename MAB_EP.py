...
def calculate_reward(max_delay, max_area, delay, area):
    #abc_cmd = "read %s;read %s; map; write temp.blif; read 7nm_lvt_ff.lib;read -m temp.blif; ps; topo; upsize; dnsize; stime; " % (genlib_origin, design)
    #res = subprocess.check_output(('abc', '-c', abc_cmd))
    #match_d = re.search(r"Delay\s*=\s*([\d.]+)\s*ps", str(res))
    #match_a = re.search(r"Area\s*=\s*([\d.]+)", str(res))
...
class EpsilonGreedyMAB:
  ...
  def select_action(self):
    ...
    # Exploration (select K cells randomly)
    #num_cells_to_explore = max(1, int(self.epsilon * self.sample_gate)) # Ensure at least one
    #while len(selected_cells) < num_cells_to_explore:
    #    selected_cells.add(random.randint(0, self.num_arms - 1))
    #print(selected_cells)

    # Exploitation (choose remaining based on Q-value)
    ...
    #        if self.counts[cell] > 0:
                #average_reward = self.q_values[cell] / self.counts[cell] #remove the division part
    #            average_reward = self.q_values[cell]
    #            if random.random() < average_reward:
    #                selected_cells.add(cell)
    #        else:
    #            selected_cells.add(cell)
    #        remaining_cells.remove(cell)
    #    else:
            # Fallback while Ensure unique selection
    #        while True:
    #            cell = random.randint(0, self.num_arms - 1)
